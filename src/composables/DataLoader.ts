import { DrawName, Draw, Candidates } from "./Constant.ts";
...
function convertDrawName(rawName: string): DrawName {
    if (rawName == "General" || rawName == "No Program Specified") {
        return DrawName.GENERAL;
        ...
        return DrawName.STEM_2023;
    } else if (rawName == "Healthcare occupations (2023-1)") {
        return DrawName.HEALTH_2023;
    } else if (rawName == "French language proficiency (2023-1)") {
        return DrawName.FRENCH_2023;
    } else if (rawName == "Trade occupations (2023-1)") {
        return DrawName.TRADE_2023;
    } else if (rawName == "Transport occupations (2023-1)") {
        return DrawName.TRANSPORT_2023;
    } else if (rawName == "Agriculture and agri-food occupations (2023-1)") {
        return DrawName.AGRICULTURE_2023;
    } else {
        return DrawName.UNKNOWN;
    }
}

function createPool(rawPoolData: Array<string>): Array<Candidates> {
    const minArr = [
        0, 301, 351, 401, 411, 421, 431, 441, 451, 461, 471, 481, 491, 501, 601,
    ];
    const maxArr = [
        300, 350, 400, 410, 420, 430, 440, 450, 460, 470, 480, 490, 500, 600,
        1200,
    ];
